...
coreaudio_error:
        ...
        return enqueue_buf(ao, buf, bufsize, samples, ao->samplerate, false);
    }
}
...
static int get_volume(struct ao *ao, struct ao_control_vol *vol) API_AVAILABLE(macos(10.13), ios(11.0))
{
    struct priv *p = ao->priv;
    float volume = p->renderer.volume;
    vol->left = vol->right = volume * 100.0;
    return CONTROL_TRUE;
}

static int set_volume(struct ao *ao, struct ao_control_vol *vol) API_AVAILABLE(macos(10.13), ios(11.0))
{
    struct priv *p = ao->priv;
    p->renderer.volume = (vol->left + vol->right) / 200.0;
    return CONTROL_TRUE;
}
